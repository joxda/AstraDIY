/*******************************************************************************
  ...
  Copyright(c) 2024-2024 D.Germa (I did modify the library from Radek to make a specialiszation for AstrAlim)
...
// We declare an auto pointer to AstrAlimFocuser.
std::unique_ptr<AstrAlimFocuser> astralimFocuser(new AstrAlimFocuser());
...
void ISInit()
{
	...
	if (isInit == 1)
	return;
	if(astralimFocuser.get() == 0)
	{
		isInit = 1;
		astralimFocuser.reset(new AstrAlimFocuser());
	}
}
...
{
	ISInit();
	astralimFocuser->ISGetProperties(dev);
}

void ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int num)
{
	ISInit();
	astralimFocuser->ISNewSwitch(dev, name, states, names, num);
}

void ISNewText(	const char *dev, const char *name, char *texts[], char *names[], int num)
{
	ISInit();
	astralimFocuser->ISNewText(dev, name, texts, names, num);
}

void ISNewNumber(const char *dev, const char *name, double values[], char *names[], int num)
{
	ISInit();
	astralimFocuser->ISNewNumber(dev, name, values, names, num);
}
...
{
	astralimFocuser->ISSnoopDevice(root);
}
...
bool AstrAlimFocuser::Connect()
{
	...
	gpiod_line_request_output(gpio_step, "step@astralim_focuser", 0);
	gpiod_line_request_output(gpio_sleep, "sleep@astralim_focuser", 1); // start stepper in wake up state
	gpiod_line_request_output(gpio_m1, "m1@astralim_focuser", 0);
	gpiod_line_request_output(gpio_m2, "m2@astralim_focuser", 0);

	// If A4988 controller, use additional GPIO
	if ( MotorBoardS[1].s == ISS_ON )
		gpiod_line_request_output(gpio_m3, "m3@astralim_focuser", 0);
	...
	DEBUG(INDI::Logger::DBG_SESSION, "AstrAlim Focuser connected successfully.");
...
void AstrAlimFocuser::ISGetProperties (const char *dev)
{
		...
		*/
		...
		switch(res)
		{
			case 1:	// 1:1
				...
				break;
			case 2:	// 1:2
				gpiod_line_request_output(gpio_m1, "m1@astralim_focuser", 1);
				gpiod_line_request_output(gpio_m2, "m2@astralim_focuser", 0);
				break;
			case 4:	// 1:4
				gpiod_line_request_output_flags(gpio_m1, "m1@astralim_focuser", GPIOD_LINE_REQUEST_FLAG_OPEN_DRAIN, 0);
				gpiod_line_request_output(gpio_m2, "m2@astralim_focuser", 0);
				break;
			case 8:	// 1:8
				gpiod_line_request_output(gpio_m1, "m1@astralim_focuser", 0);
				gpiod_line_request_output(gpio_m2, "m2@astralim_focuser", 1);
				break;
			case 16:	// 1:16
				gpiod_line_request_output(gpio_m1, "m1@astralim_focuser", 1);
				gpiod_line_request_output(gpio_m2, "m2@astralim_focuser", 1);
		...
		*/
		...
		switch(res)
		{
			...
			case 16:	// 1:16
				gpiod_line_request_output(gpio_m1, "m1@astralim_focuser", 1);
				gpiod_line_request_output(gpio_m2, "m2@astralim_focuser", 1);
				gpiod_line_request_output(gpio_m3, "m3@astralim_focuser", 1);
				break;
			default:	// 1:1
				gpiod_line_request_output(gpio_m1, "m1@astralim_focuser", 0);
				gpiod_line_request_output(gpio_m2, "m2@astralim_focuser", 0);
...
void AstrAlimFocuser::getFocuserInfo()
{
